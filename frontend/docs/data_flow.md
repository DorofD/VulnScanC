...
3.  **API Interaction**: Components use custom hooks to trigger actions. These hooks (or the components themselves) call methods from the `services/` layer. The `services/` layer uses `authFetch` for authenticated requests to communicate with the Flask backend.
