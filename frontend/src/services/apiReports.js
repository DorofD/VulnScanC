export async function apiGetOsvReport(snapshot_id, severities, project_name, snapshot_datetime) { 
    try {
        ...
        const response = await fetch(`${process.env.BACKEND_URL}/reports?type=osv&snapshot_id=${snapshot_id}&severities=${severitiesString}`, { 
            method: 'GET', 
        });
...
export async function apiGetBduReport(snapshot_id, severities, project_name, snapshot_datetime) { 
    try {
        const severitiesString = severities.join(''); 
        const response = await fetch(`${process.env.BACKEND_URL}/reports?type=bdu&snapshot_id=${snapshot_id}&severities=${severitiesString}`, { 
            method: 'GET', 
        });
...
export async function apiGetSvacerReport(project_name, project_id, branch_id, snapshot_id, snapshot_name) { 
    try {
        const response = await fetch(`${process.env.BACKEND_URL}/reports?type=svacer&project_name=${project_name}&project_id=${project_id}&branch_id=${branch_id}&snapshot_id=${snapshot_id}`, { 
            method: 'GET', 
        });
        if (!response.ok) {
            throw new Error('Network response was not ok');
...
export async function apiGetDependencyTrackReport(project_uuid, project_name) { 
    try {
        const response = await fetch(`${process.env.BACKEND_URL}/reports?type=dependency_track&project_uuid=${project_uuid}`, { 
            method: 'GET', 
        });
        if (!response.ok) {
            throw new Error('Network response was not ok');
