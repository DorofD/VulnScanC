...
export async function apiGetOsvReport(snapshot_id, severities, project_name, snapshot_datetime) {
    try {
        ...
        const response = await authFetch(`${process.env.BACKEND_URL}/reports?report_type=osv&snapshot_id=${snapshot_id}&severities=${severitiesString}`, {
            method: 'GET',
        });
...
export async function apiGetBduReport(snapshot_id, severities, project_name, snapshot_datetime) {
    try {
        const severitiesString = severities.join('');
        const response = await authFetch(`${process.env.BACKEND_URL}/reports?report_type=bdu&snapshot_id=${snapshot_id}&severities=${severitiesString}`, {
            method: 'GET',
        });
...
export async function apiGetBitbakeReport(snapshot_id, severities, layers, project_name, snapshot_datetime) {
    try {
        ...
        const response = await authFetch(`${process.env.BACKEND_URL}/reports?report_type=bitbake&snapshot_id=${snapshot_id}&severities=${severitiesString}&layers=${layersString}`, {
            method: 'GET',
        });
...
export async function apiGetBitbakeBduReport(snapshot_id, severities, layers, project_name, snapshot_datetime) {
    try {
        const severitiesString = severities.join(',');
        const layersString = layers.join(',');
        const response = await authFetch(`${process.env.BACKEND_URL}/reports?report_type=bitbake_bdu&snapshot_id=${snapshot_id}&severities=${severitiesString}&layers=${layersString}`, {
            method: 'GET',
        });
...
export async function apiGetDependencyTrackReport(project_uuid, project_name) {
    try {
        const response = await authFetch(`${process.env.BACKEND_URL}/reports?report_type=dependency_track&project_uuid=${project_uuid}`, {
            method: 'GET',
        });
        if (!response.ok) {
            throw new Error('Network response was not ok');
