...
class DOCX_GENERATOR:
    def __init__(self):
        self.tblBorders = (r'<w:tblBorders %s>'
        ...
        def create_vulnerabilities_table(doc, vulnerabilities):
            ...
            for vuln in vulnerabilities:
                ...
                for reference in dict_vuln['references']:
                    ...
                    break  # ссылки занимают много места в документе, достаточно будет одной
    ...
    def create_bdu_report(self, snapshot_id: int, severities_string: str):
        ...
        def create_vulnerabilities_table(doc, vulnerabilities):
            header_component_values = [
                'ID', 'Информация об уязвимости', 'Статус', 'Severity']
            last_paragraph = doc.add_paragraph()
            last_paragraph.paragraph_format.space_after = Cm(1)
            table = doc.add_table(rows=1, cols=4)
            table.autofit = False
            ...
            for vuln in vulnerabilities:
                table.add_row()
                body_cells = table.rows[count].cells
                body_cells[0].text = vuln['bdu_id']
                body_cells[1].text = vuln['description']
                body_cells[2].text = vuln['status']
                body_cells[3].text = vuln['bdu_severity']
        ...
        for component in components:
            if 'bdu_vulnerabilities' not in component or component['status'] != 'confirmed':
                continue
            sorted_vulnerabilities = sort_vulnerabilities(
                component['bdu_vulnerabilities'], severities_string)
            component['bdu_vulnerabilities'] = sorted_vulnerabilities
            if 'bdu_vulnerabilities' in component and component['bdu_vulnerabilities'] != []:
                create_component_table(doc, component, count)
                create_vulnerabilities_table(
                    doc, component['bdu_vulnerabilities'])
            count += 1
    ...
    def create_dependency_track_report(self, project_uuid: str):
        dt = DT()
    ...
    def create_svacer_report(self, project_name, names=[], ids=[]):
